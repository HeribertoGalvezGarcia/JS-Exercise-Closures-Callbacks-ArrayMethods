...
function processContains(item, list, callback) {
  return callback(list.filter(i => i === item).length !== 0);
...
function processDuplicateFree(list, callback) {
  return callback(list.filter((item, index, array) => array.indexOf(item) === index));
...
function tallyUpDonations(runners) {
  return runners.reduce((accumulator, {donation}) => accumulator + donation, 0);
}
